...
+ (void)GETWithURL:(NSString *)url withBody:(NSDictionary *)body withHttpHead:(NSDictionary *)head responseStyle:(responseStyle)style withSuccess:(void(^)(id result))success withFail:(void(^)(id result))fail {
    ...
    [manager GET:url parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        NSDictionary *dic = [JYJSON dictionaryOrArrayWithJSONSData:responseObject];
...
+ (void)GETLocalCityWithURL:(NSString *)url withBody:(NSDictionary *)body withHttpHead:(NSDictionary *)head responseStyle:(responseStyle)style withSuccess:(void(^)(id result))success withFail:(void(^)(id result))fail {
    ...
    NSArray *path = NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES);
    NSString *pathString = path.lastObject;
    NSString *pathLast =[NSString stringWithFormat:@"/Caches/com.localCity.get.default/%lu.text", (unsigned long)[url_string hash]];
    
    //创建字符串文件存储路径
    NSString *PathName =[pathString stringByAppendingString:pathLast];
    
    //第一次进入判断有没有文件夹，如果没有就创建一个
    NSString * textPath = [pathString stringByAppendingFormat:@"/Caches/com.localCity.get.default"];
    if (![[NSFileManager defaultManager]fileExistsAtPath:textPath]) {
